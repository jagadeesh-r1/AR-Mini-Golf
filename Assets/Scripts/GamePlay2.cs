...
public class GamePlay2 : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        {
            rb.velocity = new Vector3(0, 0); //ENABLE THIS IF YOU WANT THE BALL TO STOP IF ITS MOVING SO SLOW
            canShoot = true;
        }
    }
    ...
    void LookAtShootDirection()
    {
        ...
        if (dis < maxPower)
        {
            direction.localPosition = new Vector3(dis / 6, 0);
            shootPower = dis;
        }
        else
        {
            shootPower = maxPower;
            direction.localPosition = new Vector3(maxPower / 6, 0);
        }
